...
class GraphMixer(TGNNBackbone):
    ...
    def __init__(self, num_nodes: int, node_dim: int, edge_dim: int,
                 time_feat_dim: int, num_tokens: int, num_layers: int = 2, time_gap=2000, token_dim_expansion_factor: float = 0.5,
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
        ...
        src_node_embeddings = self.compute_node_temporal_embeddings(node_ids=src_node_ids, node_interact_times=node_interact_times,
                                                                    num_neighbors=num_neighbors, time_gap=self.time_gap, subgraphs=src_subgraphs)
        ...
        dst_node_embeddings = self.compute_node_temporal_embeddings(node_ids=dst_node_ids, node_interact_times=node_interact_times,
                                                                    num_neighbors=num_neighbors, time_gap=self.time_gap, subgraphs=dst_subgraphs)

        return src_node_embeddings, dst_node_embeddings

    def compute_node_temporal_embeddings(self, node_ids: np.ndarray, node_interact_times: np.ndarray, subgraphs: BatchSubgraphs,
        ...
        neighbor_node_ids, neighbor_edge_ids, neighbor_times, nodes_edge_raw_features, edge_attn, node_attentions = subgraphs.get_split_for_layer(0)
        ...
        time_gap_neighbor_node_ids = subgraphs.nodes[0][:,:time_gap]
        time_gap_neighbor_attention = subgraphs.node_attention[0][:,:time_gap]

        # Tensor, shape (batch_size, time_gap, node_feat_dim)
        nodes_time_gap_neighbor_node_raw_features = self.node_features[torch.from_numpy(time_gap_neighbor_node_ids)]
        nodes_time_gap_neighbor_node_raw_features = self.mask_neighbors(time_gap_neighbor_attention[:,:,None], nodes_time_gap_neighbor_node_raw_features)
        ...
        node_feat = self.node_features[torch.from_numpy(node_ids)]
        output_node_features = nodes_time_gap_neighbor_node_agg_features + node_feat
