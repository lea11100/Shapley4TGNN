...
class CAWN(TGNNBackbone):
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
        ...
        self.position_encoder.count_nodes_appearances(src_node_ids=src_node_ids, dst_node_ids=dst_node_ids,
                                                      ...
                                                      src_node_multi_hop_graphs=src_subgraphs,
                                                      dst_node_multi_hop_graphs=dst_subgraphs)
        ...
        src_node_embeddings = self.compute_node_temporal_embeddings(node_ids=src_node_ids, node_interact_times=node_interact_times,
                                                                    node_multi_hop_graphs=src_subgraphs, num_neighbors=num_neighbors)
        # Tensor, shape (batch_size, node_feat_dim)
        dst_node_embeddings = self.compute_node_temporal_embeddings(node_ids=dst_node_ids, node_interact_times=node_interact_times,
                                                                    node_multi_hop_graphs=dst_subgraphs, num_neighbors=num_neighbors)

        return src_node_embeddings, dst_node_embeddings

    def compute_node_temporal_embeddings(self, node_ids: np.ndarray, node_interact_times: np.ndarray, node_multi_hop_graphs: BatchSubgraphs, num_neighbors: int = 20):
        """
        ...
        nodes_neighbor_ids, nodes_edge_ids, nodes_neighbor_times, edge_features, node_attn = \
            self.convert_format_from_tree_to_array(node_ids=node_ids, node_interact_times=node_interact_times,
                                                   subgraph=node_multi_hop_graphs, num_neighbors=num_neighbors)
        ...
        neighbor_raw_features = self.node_features[torch.from_numpy(nodes_neighbor_ids)]
        neighbor_raw_features = torch.mul(neighbor_raw_features, node_attn[:,:,:,None])
    ...
    def convert_format_from_tree_to_array(self, node_ids: np.ndarray, node_interact_times: np.ndarray, subgraph: BatchSubgraphs, num_neighbors: int = 20):
        """
        ...
        nodes_neighbor_ids, nodes_edge_ids, nodes_neighbor_times, edge_feat, node_attn = subgraph.nodes, subgraph.events, subgraph.timestamps, subgraph.event_features, subgraph.node_attention
        ...
        node_attn = [torch.ones((len(node_ids), 1), device = self.device)] + node_attn
        ...
        edge_feat = [torch.zeros((len(node_ids), 1, edge_feat[0].shape[2]), device=self.device)] + edge_feat
        ...
        array_format_data = torch.zeros((subgraph.events[0].shape[0], subgraph.events[-1].shape[1], len(edge_feat), self.edge_feat_dim), device=self.device)
        for i, l in enumerate(edge_feat):
            array_format_data[:,:,i,:] = l.repeat(1,subgraph.events[-1].shape[1] // l.shape[1], 1)

        node_attn_data = torch.zeros((node_attn[0].shape[0], node_attn[-1].shape[1], len(node_attn)), device=self.device)
        for i, l in enumerate(node_attn):
            node_attn_data[:,:,i] = l.repeat(1, node_attn[-1].shape[1] // l.shape[1])

        # three ndarrays with shape (batch_size, num_neighbors ** self.walk_length, self.walk_length + 1)
        return array_format_data_list[0], array_format_data_list[1], array_format_data_list[2], array_format_data, node_attn_data
...
class PositionEncoder(nn.Module):
    ...
    def count_nodes_appearances(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray,
                                src_node_multi_hop_graphs: BatchSubgraphs, dst_node_multi_hop_graphs: BatchSubgraphs):
        """
        ...
        src_nodes_neighbor_ids, src_nodes_neighbor_times = src_node_multi_hop_graphs.nodes, src_node_multi_hop_graphs.timestamps
        ...
        dst_nodes_neighbor_ids, dst_nodes_neighbor_times = dst_node_multi_hop_graphs.nodes, dst_node_multi_hop_graphs.timestamps
...
class BiLSTMEncoder(nn.Module):
    ...
    def forward(self, inputs: torch.Tensor, lengths: np.ndarray):
        ...
        inputs = pack_padded_sequence(inputs, lengths.flatten(), batch_first=True, enforce_sorted=False) # type: ignore
