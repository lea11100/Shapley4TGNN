...
from .modules import TimeEncoder, TransformerEncoder, TGNNBackbone
from ..utils.utils import NeighborSampler, BatchSubgraphs

class TCL(TGNNBackbone):
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
        ...
        src_neighbor_node_ids, src_neighbor_edge_ids, src_neighbor_times, src_edge_features, src_edge_attn, src_node_attn = src_subgraphs.get_split_for_layer(0)

        dst_neighbor_node_ids, dst_neighbor_edge_ids, dst_neighbor_times, dst_edge_features, dst_edge_attn, dst_node_attn = dst_subgraphs.get_split_for_layer(0)
        ...
        src_edge_attn = nn.functional.pad(src_edge_attn, (1,0,0,0))
        src_node_attn = nn.functional.pad(src_node_attn, (1,0,0,0), value = 1)
        src_nodes_edge_raw_features = nn.functional.pad(src_edge_features, (0,0,1,0,0,0))
        ...
        dst_edge_attn = nn.functional.pad(dst_edge_attn, (1,0,0,0))
        dst_node_attn = nn.functional.pad(dst_node_attn, (1,0,0,0), value = 1)
        dst_nodes_edge_raw_features = nn.functional.pad(dst_edge_features, (0,0,1,0,0,0))
        ...
        src_nodes_neighbor_node_raw_features, src_nodes_neighbor_time_features, src_nodes_neighbor_depth_features = \
        ...
        src_nodes_neighbor_node_raw_features = self.mask_neighbors(src_node_attn, src_nodes_neighbor_node_raw_features)
        ...
        dst_nodes_neighbor_node_raw_features, dst_nodes_neighbor_time_features, dst_nodes_neighbor_depth_features = \
        ...
        dst_nodes_neighbor_node_raw_features = self.mask_neighbors(dst_node_attn, dst_nodes_neighbor_node_raw_features)
        ...
        src_node_embeddings = torch.tensor(0)
        dst_node_embeddings = torch.tensor(0)

        for transformer in self.transformers:
            # self-attention block
            # Tensor, shape (batch_size, num_neighbors + 1, node_feat_dim)
            src_node_features = transformer(inputs_query=src_node_features, inputs_key=src_node_features,
                                            inputs_value=src_node_features, neighbor_masks=src_neighbor_node_ids, edge_attn=src_edge_attn)
            # Tensor, shape (batch_size, num_neighbors + 1, node_feat_dim)
            dst_node_features = transformer(inputs_query=dst_node_features, inputs_key=dst_node_features,
                                            inputs_value=dst_node_features, neighbor_masks=dst_neighbor_node_ids, edge_attn=dst_edge_attn)
    ...
    def get_features(self, node_interact_times: np.ndarray, nodes_neighbor_ids: np.ndarray, nodes_edge_ids: np.ndarray,
        ...
        """
        # Tensor, shape (batch_size, num_neighbors + 1, node_feat_dim)
        nodes_neighbor_node_raw_features = self.node_features[torch.from_numpy(nodes_neighbor_ids)]
        ...
        return nodes_neighbor_node_raw_features, nodes_neighbor_time_features, nodes_neighbor_depth_features
