...
from ..utils.utils import NeighborSampler, BatchSubgraphs, concat_subgraphs
from .modules import TimeEncoder, MergeLayer, MultiHeadAttention, TGNNBackbone

class MemoryModel(TGNNBackbone):
    ...
    def __init__(self, num_nodes: int, node_dim: int, edge_dim: int,
        ...
        if self.model_name == 'JODIE':
            ...
            self.embedding_module = GraphAttentionEmbedding(time_encoder=self.time_encoder,
    ...
    def compute_src_dst_node_temporal_embeddings(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray,
                                                 ...
                                                 num_neighbors = 20,
                                                 edges_are_positive: bool = True, edge_features: Optional[torch.Tensor] = None, 
                                                 **kwargs):
        """
        ...
        subgraphs = concat_subgraphs([src_subgraphs, dst_subgraphs])
        subgraphs.reverse_layers()
        ...
        if self.model_name == 'JODIE':
            ...
            node_embeddings = typing.cast(TimeProjectionEmbedding, self.embedding_module).compute_node_temporal_embeddings(node_memories=updated_node_memories,
                                                                                     node_ids=node_ids,
                                                                                     node_time_intervals=node_time_intervals)
        elif self.model_name in ['TGN', 'DyRep']:
            # Tensor, shape (2 * batch_size, node_feat_dim)
            node_embeddings = typing.cast(GraphAttentionEmbedding, self.embedding_module).compute_node_temporal_embeddings(node_memories=updated_node_memories,
                                                                                     node_ids=node_ids,
                                                                                     node_interact_times=np.concatenate([node_interact_times,
                                                                                                                         node_interact_times]),
                                                                                     current_layer_num=self.num_layers,
                                                                                     num_neighbors=num_neighbors, subgraphs = subgraphs)
        else:
            raise ValueError(f'Not implemented error for model_name {self.model_name}!')
    ...
    def compute_new_node_raw_messages(self, src_node_ids: np.ndarray, dst_node_ids: np.ndarray, dst_node_embeddings: torch.Tensor,
                                      node_interact_times: np.ndarray, edge_features: torch.Tensor):
        """
...
class MemoryUpdater(nn.Module):
    ...
    def __init__(self, memory_bank: MemoryBank):
        ...
        self.memory_updater = nn.GRUCell(input_size=0, hidden_size=0)
...
class GraphAttentionEmbedding(nn.Module):
    ...
    def __init__(self, time_encoder: TimeEncoder, node_feat_dim: int, edge_feat_dim: int, time_feat_dim: int,
                 num_layers: int = 2, num_heads: int = 2, dropout: float = 0.1, device = "cpu"):
        """
    ...
    def compute_node_temporal_embeddings(self, node_memories: torch.Tensor, node_ids: np.ndarray, node_interact_times: np.ndarray, subgraphs: BatchSubgraphs,
        ...
        node_time_features = self.time_encoder(timestamps=torch.zeros(node_interact_times.shape).unsqueeze(dim=1).to(self.device))
        ...
        node_features = node_memories[torch.from_numpy(node_ids)]
        ...
        if current_layer_num <= 0:
            ...
            node_conv_features = self.compute_node_temporal_embeddings(node_memories=node_memories,
                                                                       node_ids=node_ids,
                                                                       node_interact_times=node_interact_times,
                                                                       current_layer_num=current_layer_num - 2,
                                                                       num_neighbors=num_neighbors, subgraphs=subgraphs)
            ...
            neighbor_node_ids, neighbor_edge_ids, neighbor_times, neighbor_edge_features, edge_attn, node_attn = subgraphs.get_split_for_layer(current_layer_num-1, flat_to_node=True)
            ...
            neighbor_node_conv_features = torch.mul(neighbor_node_conv_features, node_attn[:,:,None]) 
            ...
            neighbor_time_features = self.time_encoder(timestamps=torch.from_numpy(neighbor_delta_times).float().to(self.device))
...
def compute_src_dst_node_time_shifts(src_node_ids: np.ndarray, dst_node_ids: np.ndarray, node_interact_times: np.ndarray):
    ...
    src_node_mean_time_shift = np.mean(src_node_all_time_shifts).item()
    src_node_std_time_shift = np.std(src_node_all_time_shifts).item()
    dst_node_mean_time_shift_dst = np.mean(dst_node_all_time_shifts).item()
    dst_node_std_time_shift = np.std(dst_node_all_time_shifts).item()
