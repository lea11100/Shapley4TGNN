...
from DyGLib.utils.metrics import get_link_prediction_metrics, get_node_classification_metrics, get_link_regression_metrics
from DyGLib.utils.utils import set_random_seed, BatchSubgraphs, NegativeEdgeSampler, NeighborSampler
...
def evaluate_model_link_prediction(model: nn.Module, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                   ...
                                   num_neighbors: int = 20, num_layers=2, time_gap: int = 2000, device="cpu"):
    """
    ...
    with torch.no_grad():
        ...
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            ...
            batch_edge_features = neighbor_sampler.edge_features[batch_edge_ids].to(model.backbone.device)
            ...
            subgraphs_src = neighbor_sampler.get_multi_hop_neighbors(num_layers, batch_src_node_ids, batch_node_interact_times, num_neighbors = num_neighbors)
            subgraphs_dst = neighbor_sampler.get_multi_hop_neighbors(num_layers, batch_dst_node_ids, batch_node_interact_times, num_neighbors = num_neighbors)
            subgraphs_dst_neg = neighbor_sampler.get_multi_hop_neighbors(num_layers, batch_neg_dst_node_ids, batch_node_interact_times, num_neighbors = num_neighbors)
            edge_feat_src = neighbor_sampler.get_edge_features_for_multi_hop(subgraphs_src[1])
            edge_feat_dst = neighbor_sampler.get_edge_features_for_multi_hop(subgraphs_dst[1])
            edge_feat_dst_neg = neighbor_sampler.get_edge_features_for_multi_hop(subgraphs_dst_neg[1])

            subgraphs_src = BatchSubgraphs(*subgraphs_src, edge_feat_src)
            subgraphs_src.to(device)
            subgraphs_dst = BatchSubgraphs(*subgraphs_dst, edge_feat_dst)
            subgraphs_dst.to(device)
            subgraphs_dst_neg = BatchSubgraphs(*subgraphs_dst_neg, edge_feat_dst_neg)
            subgraphs_dst_neg.to(device)
...
def evaluate_model_link_regression(model: TGNN, neighbor_sampler: NeighborSampler, evaluate_idx_data_loader: DataLoader,
                                       evaluate_data: Data, loss_func: nn.Module, num_neighbors: int = 20, num_layers=2, time_gap: int = 2000, device="cpu"):
    """
    evaluate models on the node classification task
    :param model_name: str, name of the model
    :param model: nn.Module, the model to be evaluated
    ...
    with torch.no_grad():
        # store evaluate losses, trues and predicts
        evaluate_y_trues, evaluate_y_predicts = [], []
        edge_ids = []
        evaluate_idx_data_loader_tqdm = tqdm(evaluate_idx_data_loader, ncols=120)
        for batch_idx, evaluate_data_indices in enumerate(evaluate_idx_data_loader_tqdm):
            evaluate_data_indices = evaluate_data_indices.numpy()
            ...
            not_nan_mask = ~torch.isnan(labels)
            loss = torch.tensor(np.nan)
            if(not_nan_mask.any()):
                subgraphs_src = neighbor_sampler.get_multi_hop_neighbors(num_layers, batch_src_node_ids[not_nan_mask], batch_node_interact_times[not_nan_mask], num_neighbors = num_neighbors)
                subgraphs_dst = neighbor_sampler.get_multi_hop_neighbors(num_layers, batch_dst_node_ids[not_nan_mask], batch_node_interact_times[not_nan_mask], num_neighbors = num_neighbors)
                edge_feat_src = neighbor_sampler.get_edge_features_for_multi_hop(subgraphs_src[1])
                edge_feat_dst = neighbor_sampler.get_edge_features_for_multi_hop(subgraphs_dst[1])
                ...
                predicts = model(src_node_ids=batch_src_node_ids[not_nan_mask],
                ...
                evaluate_y_trues.append(labels)
                evaluate_y_predicts.append(predicts)
                edge_ids.append(batch_edge_ids[not_nan_mask])

            if(batch_idx % 50 == 0):
                evaluate_idx_data_loader_tqdm.set_description(f'evaluate for the {batch_idx + 1}-th batch, evaluate loss: {loss.item() if not torch.isnan(loss) else np.nan}')

        evaluate_y_trues = torch.cat(evaluate_y_trues, dim=0)
        evaluate_y_predicts = torch.cat(evaluate_y_predicts, dim=0)

        evaluate_metrics = get_link_regression_metrics(predicts=evaluate_y_predicts, true_value=evaluate_y_trues)
