...
def get_effect_eval_data(node_dim: int):
    """
    obtain Effect Evaluation data
    ...
    assert node_dim == node_raw_features.shape[1], 'Unaligned feature dimensions after feature padding!'

    # get the timestamp of validate and test set
    val_time, test_time = 16254, 16353

    src_node_ids = graph_df.u.values.astype(np.longlong)
    dst_node_ids = graph_df.i.values.astype(np.longlong)
    node_interact_times = graph_df.ts.values.astype(np.float32)
    edge_ids = graph_df.idx.values.astype(np.longlong)
    labels = graph_df.label.to_numpy()
    types = graph_df.type.to_numpy()
    ...
    print("The splits are at {} and {} (including).".format(val_time, test_time))
