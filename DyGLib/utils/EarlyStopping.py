...
from DyGLib.models.MemoryModel import MemoryModel
...
class EarlyStopping(object):
    ...
    def step(self, metrics: list, model: nn.Module, interm_results: Optional[dict] = None, save_model_folder: str = "", model_name=""):
        """
        ...
        if torch.all(torch.tensor(metrics_compare_results)):
            ...
            if interm_results is not None:
                val_metric_dict = {}
                for metric_name in interm_results.keys():
                    val_metric = interm_results[metric_name]
                    val_metric_dict[metric_name] = val_metric
                val_json = {
                        "validate metrics": {metric_name: f'{val_metric_dict[metric_name]:.4f}' for metric_name in val_metric_dict},
                    }
                
                val_json = json.dumps(val_json, indent=4)

                save_result_folder = save_model_folder
                os.makedirs(save_result_folder, exist_ok=True)
                save_result_path = os.path.join(save_result_folder, f"Intermediate_{model_name}.json")
    ...
    def save_checkpoint(self, model: nn.Module):
        ...
        if isinstance(model.backbone, MemoryModel):
            torch.save(model.backbone.memory_bank.node_raw_messages, self.save_model_nonparametric_data_path)
    ...
    def load_checkpoint(self, model: nn.Module, map_location: str = None):
        ...
        if isinstance(model.backbone, MemoryModel):
            model.backbone.memory_bank.node_raw_messages = torch.load(self.save_model_nonparametric_data_path, map_location=map_location, weights_only=False)
