...
# sys.path.append(p)
from Explainers.External.tgnnexplainer.tgnnexplainer.tgnnexplainer.__init__ import ROOT_DIR
...
def generate_explain_index(file, explainer_idx_dir, dataset_name, explain_idx_name=None):
    ...
    elif dataset_name in ['wikipedia', 'reddit', 'mooc', 'reddit_v1', 'reddit_hyperlinks']:
        np.random.seed(1024)
        e_num = len(df)
        start_ratio = 0.7
        end_ratio = 0.99
