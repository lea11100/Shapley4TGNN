...
pruned_dir = Path(modified_dir).parents[1]
...
def pipeline(config: DictConfig):
    ...
    config.models.ckpt_path = str(
        ...
        / "models"
        / "ext"
        / f"{config.models.model_name}"
        / "saved_models"
        / f"{config.models.model_name}_{config.datasets.dataset_name}_{config.seed}_best.pth"
    )
    ...
    if (
        ...
        explainer = [
            SubgraphXTG(
                ...
                pg_explainer_model=(
                    pg_explainer_model if config.explainers.use_pg_explainer else None
                ),
    ...
    if config.explainers.explainer_name == "subgraphx_tg":
        ...
        evaluator = EvaluatorMCTSTG(
            ...
            results_dir=config.explainers.results_dir,
            seed=config.seed,
            cpuct=config.explainers.param.c_puct
        )
